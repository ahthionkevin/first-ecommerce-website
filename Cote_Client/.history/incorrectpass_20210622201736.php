   ...
   if(!empty($_POST['recup_mail'])) {
      ...
      if(filter_var($recup_mail,FILTER_VALIDATE_EMAIL)) {
         ...
         if($mailexist_count == 1) {
            ...
            $mail_recup_exist = $bdd->prepare('SELECT id FROM client WHERE mail = ?');
            $mail_recup_exist->execute(array($recup_mail));
            $mail_recup_exist = $mail_recup_exist->rowCount();
            if($mail_recup_exist == 1) {
               $recup_insert = $bdd->prepare('UPDATE client SET code = ? WHERE mail = ?');
               $recup_insert->execute(array($recup_code,$recup_mail));
            } else {
               $recup_insert = $bdd->prepare('INSERT INTO client(mail,code) VALUES (?, ?)');
         ...
         mail($recup_mail, "Récupération de mot de passe - Votresite", $message, $header);
            header("Location:http://127.0.0.1/path/client.php?section=code");
         } else {
...
if(isset($_POST['verif_submit'],$_POST['verif_code'])) {
   if(!empty($_POST['verif_code'])) {
      ...
      $verif_req = $bdd->prepare('SELECT id FROM client WHERE mail = ? AND code = ?');
